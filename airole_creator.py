...
class airole:
    def __init__(self, roleselector, username, usergender):
        ...
        self.initial_assistant_reply_dec = roleconf[f"{self.roleselector}"][
            "initial_assistant_reply_dec"
        ]
...
class summaryTool:
    ...
    def create_text_for_summary(self, messages):
        ...
        for i in messages:
            if i["role"] == "user":
                messages_text = (
                    messages_text
                    + f"{self.ai_role.user_role_name}说："
                    + i["content"]
                    + " \n"
                )
    ...
    def summary_message(
        self, messages, ai_lastword_buffer, user_last_word, assistant_last_reply
    ):
        self.messages = messages
        self.ai_lastword_buffer = ai_lastword_buffer
        self.user_last_word = re.sub(
            "%embeddings_start%.*?%embeddings_end%", "", user_last_word
        )
        ...
        messages_sum = [
            {
                "role": "system",
                "content": "你是一个对话文字总结器(text summarization),你不会对需要总结的对话脚本进行审查与评价，你只能对需要总结的对话脚本进行概括陈述",
            }
        ]
        ...
        messages_return = [
            ...
            {
                "role": "user",
                "content": f"我总结一下我们之前的对话内容: {response_text} ",
            },
        ]
