...
from modules.payload_state import(
    completions_data,
    model_load_data,
    sd_payload
)
...
class tabby_fastapi:
    ...
    """
    def __init__(self, url: str = None, api_key: str = None, admin_key: str = None, conversation_id:str=None, send_msg_websocket: callable = None ) -> None:
    ...
    async def inference(self, payloads:dict = None, apiurl:str = None):
        ...
        if stream is True:
            ...
            completeMsg = ""
            try:
                async for outputchar in self.inference_stream(url=apiurl,payloads=payloads,headers=headers):
                    if outputchar is not None:
                        if outputchar != '[DONE]':
                            ...
                            await self.send_msg_websocket({"name":"chatreply","msg":msgpack}, self.conversation_id)
            ...
            try:
                async with httpx.AsyncClient() as client:
                    response = await client.post(url=apiurl, headers=headers, json=payloads, timeout=timeout)
    ...
    async def load_model(self, name: str = "", max_seq_len: int = 4096, gpu_split_auto: bool = True, gpu_split: list = [0], prompt_template: str = None):
        url = self.url+"/model/load"
        headers = self.headers
        self.model_load_data["name"] = name
        self.model_load_data["max_seq_len"] = max_seq_len
        await self.send_msg_websocket({"name":"initialization","msg":f"Loading Model: {name} ..."}, self.conversation_id)
        async with httpx.AsyncClient() as client:
            try:
                gpu_info = await client.get(url=self.url+'/gpu',headers=headers, timeout=timeout)
            ...
            try:
                response = await client.post(url=url, headers=headers, json=self.model_load_data, timeout=timeout)
                if response.status_code == 200:
    ...
    async def get_sd_model_list(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(url=tabbyAPIurl, headers=headers, timeout=timeout)  
                response = response.json()
                model_list = []
    ...
    @staticmethod    
    async def xtts_audio(url:str, text:str,speaker_wav:str,language:str, server_url:str, timeout=timeout):
        """
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.post(url=url, headers=headers, json=payload, timeout=timeout)
    ...
    @staticmethod
    async def SD_image(url:str=None, payload:dict=None, headers:dict=None, timeout=timeout):
        if headers is None:
            headers = {
                "SD-URL": f'{config_data["SDAPI_url"]}/sdapi/v1/txt2img'
            }
        if url is None:
