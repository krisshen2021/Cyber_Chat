...
monkey.patch_all()
import uuid, markdown, requests, base64, json
...
from modules.DelChatRoom import DelRoom
...
def index():
    ...
    for key, value in roleconf.items():
        ...
        ai_is_uncensored.append(
            value.get("if_uncensored")
        )  # read the uncensored flag list
...
def enter_room():
    if request.method == "POST":
        facelooks = json.loads(request.form["facelooks"])
        space = " " if facelooks["hair_color"] != "" else ""
        ends = ", " if facelooks["beard"] != "" else ""
        facelooks = f"one {facelooks['gender']}, {facelooks['race']}, {facelooks['age']}, {facelooks['hair_color']}{space}{facelooks['hair_style']}, {facelooks['eye_color']}{ends}{facelooks['beard']}"
        print(f"The User's Look is : {facelooks}")
        userdata = {
            ...
            "is_uncensored": request.form.get(
                "is_uncensored", "Yes"
            ),  # 使用 .get() 以处理缺失的字段
...
def client_signup(client_info):
    ...
    if vali_result["validated"]:
        ...
        send_status(
            {"name": "signup_validation", "msg": {"status": "Fail", "data": error}},
            conversation_id,
        )
    pass


@socketio.on("client_edit_profile")
def client_edit_profile(client_info):
    if client_info["data"]["nickname"] == "":
        client_info["data"]["nickname"] = client_info["data"]["username"]
    username = client_info["data"]["username"]
    nickname = client_info["data"]["nickname"]
    ...
    if vali_result["validated"]:
        ...
        error = vali_result["data"]
        send_status(
            {"name": "edit_validation", "msg": {"status": "Fail", "data": error}},
            conversation_id,
        )
    pass
...
def client_login(client_info):
    ...
    if vali_result["validated"]:
        ...
        error = vali_result["data"]
        send_status(
            {"name": "login_validation", "msg": {"status": "Fail", "data": error}},
            conversation_id,
        )
    pass
...
def client_disconnect():
    ...
    for conid, room in list(chatRoomList.items()):
        if room.conversation_id == user_sid:
            ...
            print(
                f">>> User {user_sid} disconnected, room {conid} is pending to remove in 5 min"
            )
...
def initializeRoom(client_msg):
    ...
    for roomid in chatRoomList.keys():
        ...
        if ai_is_uncensored == "No":
            ...
            )  # Generate a unique chatRoom with unique conid for specific user.
            print("a censored room created")
        else:
            chatRoomList[conid] = chatRoom_unsensor(
                user_sys_name=user_sys_name,
                ai_role_name=ai_role_name,
                username=username,
                usergender=usergender,
    ...
    if not userCurrentRoom.initialization_start:
        ...
        socketio.emit(
            "my response", {"data": data_to_send}, room=userCurrentRoom.conversation_id
        )
...
def reset(client_msg):
    ...
    send_status(
        {"name": "initialization", "msg": "DONE"}, userCurrentRoom.conversation_id
    )
    ...
    def callback_unload(future):
        ...
        if unloadresp.status_code == 200:
            ...
            def callback_load(future):
                ...
                if response == "Success":
                    send_status(
                        {"name": "initialization", "msg": "DONE"},
                        userCurrentRoom.conversation_id,
                    )
                    pass
                ...
                socketio.emit(
                    "model_change_result",
                    {"data": data_to_send},
                    room=userCurrentRoom.conversation_id,
                )

            future = executor.submit(
                userCurrentRoom.my_generate.tabby_server.load_model,
                name=model,
                send_msg_websocket=send_status,
            )
...
def load_chat_history(client_msg):
    conid = client_msg["data"]["conid"]
    conversation_id = client_msg["data"]["socket_id"]
    userCurrentRoom = chatRoomList[conid]
    userCurrentRoom.conversation_id = conversation_id
...
if __name__ == "__main__":
    ...
    print(
        f"Cyber Chat Server is running on http://{host}:{port} , debug mode is {debug}"
    )
